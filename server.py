...
def handle_client(sock_obj, client_addr):
    print(f"[NEW CONNECTION] {client_addr} User: {clients[sock_obj]} connected to the server..")
    connected = True
...
def start_server():
    ...
    while True:  # server starts up and runs infinitely
        ...
        # Request and store nickname and corresponding client socket info
        ...
        broadcastMessage(f"{nickname} joined!".encode(FORMAT))
